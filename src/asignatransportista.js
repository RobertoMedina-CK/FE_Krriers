...
function Asigna() {
  ...
  const selectTransportista = (val) => {
    ...
    const newSubastaList = subastaList.map((el) => {
      ...
      return {
        ...el,
        pagocarrier: Number(el.precio) * perc + Number(el.feescarrier),
      };
    });
  ...
  const getAsigna = () => {
    axios.get(`/asigna`).then((response) => {
  ...
  const getTransportistas = () => {
    axios.get(`/transportistas`).then((response) => {
      const res = response.data;
      const mutatedRes = res.map((item) => {
        return { ...item, disabled: false };
      });
  ...
  const getFileUrl = async () => {
    ...
    const response = await axios.post(`/whatsappFile`, body);
  ...
  const generapdf = () => {
    ...
    doc.text(
      `${nombretransportista} No. ${idt} el ${fechaasignacarrier}  `,
      100,
      60
    );

    autoTable(doc, {
      html: "#pedidos-seleccionados",
      margin: { top: 70, right: 30 },
      columns: [
        { header: "Subasta", dataKey: "subasta" },
        ...
        { header: "Fecha Pedido", dataKey: "fechapedido" },
      ],
    });
    ...
    const pdfFile = new File([pdfBlob], `${idt}${fechaasignacarrier}.pdf`, {
  ...
  const asignarCarga = () => {
    ...
    Axios.post(
      "https://hook.us1.make.com/gi61wipte1j5fdx7bd4b5cp9vooz960s",
      params
    )
      .then(() => {
        console.log("Success");
  ...
  return (
    <div className="container">
      <div className="card text-center">
                    ...
                    >
                      <button
                        ...
                        onClick={() => {
                          ...
                          const deselectTransportistas =
                            filteredTransportistas.map((el) => {
                              return el.disabled
                                ? { ...el, disabled: false }
                                : el;
                            });
                          const transportistaElement =
                            deselectTransportistas.map((el) => {
                              return el.id === val.id
                                ? { ...el, disabled: true }
                                : el;
                            });
        ...
        <table
          id="tabla-pedidos-no-seleccionados"
          className="table  table-hover"
          style={{
            overflowY: "scroll",
            maxHeight: "300px",
                        ...
                        onClick={() => {
                          ...
                          newSubastaList = [
                            ...newSubastaList,
                            {
                              ...val,
                              pagocarrier:
                                Number(val.precio) * pagocarrier2 +
                                Number(val.feescarrier),
                            },
                          ];
                          console.log(newSubastaList);
                          console.log(
        ...
        >
          <button
            ...
            onClick={() => asignarCarga()}
            disabled={
              subastaList.length < 1 || !idTransportista || !fechaasignacarrier
            }
          >
            Asignar Carga
          </button>
